...
pub fn read_byte(emulator: &mut Emulator, address: u16) -> u8 {
    if emulator.processor_test_mode {
        ...
        if address_accessible(emulator, address) {
            unsafe_read_byte(emulator, address)
        }
        else {
            0xFF
        }
...
pub fn unsafe_read_byte(emulator: &mut Emulator, address: u16) -> u8 {
    let byte = match address & 0xF000 {
        ...
        0x8000..=0x9FFF =>
            emulator.gpu.get_video_ram_byte(address & 0x1FFF),
        0xA000..=0xBFFF =>
        ...
        0xF000 => match address & 0x0F00 {
            ...
            0xE00 if address < 0xFEA0 => emulator.gpu.get_object_attribute_memory_byte(address & 0xFF),
            ...
            _ => match address & 0xFF {
                ...
                0x01 => emulator.serial.data(),
                0x02 => emulator.serial.control(),
                ...
                0x40 => emulator.gpu.lcdc(),
                0x41 => emulator.gpu.stat(),
                0x42 => emulator.gpu.scy(),
                0x43 => emulator.gpu.scx(),
                0x44 => emulator.gpu.ly(),
                0x45 => emulator.gpu.lyc(),
                0x46 => dma::get_source(emulator),
                0x47 => emulator.gpu.palettes_readonly().bgp(),
                0x48 => emulator.gpu.palettes_readonly().obp0(),
                0x49 => emulator.gpu.palettes_readonly().obp1(),
                0x4A => emulator.gpu.wy(),
                0x4B => emulator.gpu.wx(),
                0x4C => emulator.gpu.key0(),
                0x4D => speed_switch::get_key1(emulator),
                0x4F => emulator.gpu.cgb_vbk(),
                0x55 => hdma::get_hdma5(emulator),
                0x68 => emulator.gpu.palettes_readonly().cgb_bcps(),
                0x69 => emulator.gpu.palettes_readonly().cgb_bcpd(),
                0x6A => emulator.gpu.palettes_readonly().cgb_ocps(),
                0x6B => emulator.gpu.palettes_readonly().cgb_ocpd(),
                0x6C => emulator.gpu.cgb_opri(),
...
pub fn write_byte(emulator: &mut Emulator, address: u16, value: u8) {
    if emulator.processor_test_mode {
        ...
        if address_accessible(emulator, address) {
            unsafe_write_byte(emulator, address, value);
        } 
    }
}

pub fn unsafe_write_byte(emulator: &mut Emulator, address: u16, value: u8) {
    match address & 0xF000 {
        0x0000..=0x7FFF =>
            emulator.memory.cartridge_mapper.write_rom(address, value),
        0x8000..=0x9FFF =>
            emulator.gpu.set_video_ram_byte(address & 0x1FFF, value),
        0xA000..=0xBFFF =>
        ...
        0xF000 => match address & 0x0F00 {
            ...
            0xE00 if address < 0xFEA0 => emulator.gpu.set_object_attribute_memory_byte(address & 0xFF, value),
            ...
            _ => match address & 0xFF {
                ...
                0x01 => emulator.serial.set_data(value),
                0x02 => emulator.serial.set_control(value),
                ...
                0x40 => emulator.gpu.set_lcdc(value),
                0x41 => emulator.gpu.set_stat(value),
                0x42 => emulator.gpu.set_scy(value),
                0x43 => emulator.gpu.set_scx(value),
                0x44 => emulator.gpu.set_ly(value),
                0x45 => emulator.gpu.set_lyc(value),
                0x46 => dma::start_dma(emulator, value),
                0x47 => emulator.gpu.palettes().set_bgp(value),
                0x48 => emulator.gpu.palettes().set_obp0(value),
                0x49 => emulator.gpu.palettes().set_obp1(value),
                0x4C => emulator.gpu.set_key0(value),
                ...
                0x4A => emulator.gpu.set_wy(value),
                0x4B => emulator.gpu.set_wx(value),
                0x4F => emulator.gpu.set_cgb_vbk(value),
                0x68 => emulator.gpu.palettes().set_cgb_bcps(value),
                0x69 => emulator.gpu.palettes().set_cgb_bcpd(value),
                0x6A => emulator.gpu.palettes().set_cgb_ocps(value),
                0x6B => emulator.gpu.palettes().set_cgb_ocpd(value),
                0x6C => emulator.gpu.set_cgb_opri(value),
                0x70 => {
                    if is_cgb(emulator) {
