...
pub fn step(cpu_state: &mut CpuState) {
    ...
    execute_opcode(cpu_state);

    interrupts::step(cpu_state);
...
fn execute_opcode(cpu_state: &mut CpuState) {
    ...
    let opcode = cpu_state.registers.opcode;

    emulate_halt_bug(cpu_state);
    update_interrupt_flag_after_delay(cpu_state);
    ...
    match opcode {
        ...
        0x18 => {
            let byte = read_next_instruction_byte(cpu_state) as i8;
        ...
        0x39 => {
            ...
            alu::add_value_to_register_pair(cpu_state, REGISTER_HL, stack_pointer)
        }
        0x3A => {
            let mut address = microops::read_from_register_pair(cpu_state, &REGISTER_HL);
            loads::load_memory_byte_in_destination_register(cpu_state, address, Register::A);
        ...
        0xE8 => {
            ...
            microops::set_flag_z(cpu_state, false);
            microops::set_flag_n(cpu_state, false);
            microops::set_flag_h(cpu_state, (sum & 0xF) < (stack_pointer & 0xF));
            microops::set_flag_c(cpu_state, (sum & 0xFF) < (stack_pointer & 0xFF));
        ...
        0xF1 => {
            ...
            microops::store_in_register_pair(cpu_state, REGISTER_AF, word & 0xFFF0);
        }
        0xF2 => {
            let address = 0xFF00 + microops::read_from_register(cpu_state, &Register::C) as u16;
            loads::load_memory_byte_in_destination_register(cpu_state, address, Register::A);
