...
impl Cpu {
    ...
    pub fn conditional_relative_jump(&mut self, condition: bool) {
    ...
    pub fn conditional_jump_using_immediate_word(&mut self, condition: bool) {
    ...
    pub fn conditional_call_using_immediate_word(&mut self, condition: bool) {
        let word = self.read_next_instruction_word();
        if condition {
            let program_counter = self.registers.program_counter;
            self.push_word_to_stack(program_counter);
    ...
    pub fn conditional_stack_return(&mut self, condition: bool) {
        self.step_one_machine_cycle();
        if condition {
            self.stack_return();
        }
    ...
    pub fn restart(&mut self, new_address: u16) {
