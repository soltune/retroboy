use crate::cpu::{Register, RegisterPair, Cpu, read_next_instruction_byte};
use crate::cpu::microops;

pub fn load_immediate_value(cpu_state: &mut Cpu, register: Register) {
    let immediate_byte = read_next_instruction_byte(cpu_state);
    microops::store_in_register(cpu_state, register, immediate_byte);
}

pub fn load_source_register_in_destination_register(cpu_state: &mut Cpu, source: Register, destination: Register) {
...
pub fn load_memory_byte_in_destination_register(cpu_state: &mut Cpu, address: u16, destination: Register) {
    let byte = microops::read_byte_from_memory(cpu_state, address);
    microops::store_in_register(cpu_state, destination, byte);
}

pub fn load_source_register_in_memory(cpu_state: &mut Cpu, source: Register, address: u16) {
    let byte = microops::read_from_register(cpu_state, &source);
    microops::store_byte_in_memory(cpu_state, address, byte);
}

pub fn load_immediate_value_in_memory(cpu_state: &mut Cpu, register_pair: RegisterPair) {
...
pub fn push_word_to_stack(cpu_state: &mut Cpu, word: u16) {
...
pub fn push_register_pair_to_stack(cpu_state: &mut Cpu, register_pair: RegisterPair) {
    let word = microops::read_from_register_pair(cpu_state, &register_pair);
    push_word_to_stack(cpu_state, word);
}

pub fn pop_word_from_stack(cpu_state: &mut Cpu) -> u16 {
...
pub fn pop_word_into_register_pair_from_stack(cpu_state: &mut Cpu, register_pair: RegisterPair) {
